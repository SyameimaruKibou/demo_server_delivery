...
public class SendMesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.send_btn_save).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(Save()==true){
                    mIntent.putExtra("CustomerInfo",mItem);  //存储保存信息
                    if(addcount!=-1){
                        mIntent.putExtra("count",addcount);
                    }
        ...
        if (mIntent.hasExtra("Action")) {
            ...
            else if(mIntent.getStringExtra("Action").equals("NewAddr")){//地址首次进行编辑
                mItem = new TempCustomer();
            }
            else if(mIntent.getStringExtra("Action").equals("EdtAddr")){
                mItem = new TempCustomer();
                if (mIntent.hasExtra("CustomerInfo")) {
                    mItem = (TempCustomer) mIntent.getSerializableExtra("CustomerInfo");
                    addcount = mIntent.getIntExtra("count",-1);
                    mNameView.setText(mItem.getName());
                    //	Log.i("NAME==================",""+mNameView.getText().toString());
                    mTelCodeView.setText(mItem.getTelCode());
                    mAddrView.setText(mItem.getAddress());
